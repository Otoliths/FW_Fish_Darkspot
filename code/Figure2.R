...
p <- ggplot(data = posterior_summary, aes(x = Estimate, y = interaction(variable, group), fill = shortfall)) +
  ...
  theme(strip.background = element_blank(),  # Remove background for facet strips
        ...
        axis.text.y = element_markdown(size = 8, colour = c(rep("#A3473E", 6), rep("#6D4D80", 5), rep("#6C8F5D", 3))),  # Use markdown for y-axis text color
        ggh4x.axis.nestline.y = element_line(colour = c("#A3473E", "#6D4D80", "#6C8F5D")),  # Set color for y-axis nesting lines
        ggh4x.axis.nesttext.y = element_markdown(colour = c("#A3473E", "#6D4D80", "#6C8F5D"), angle = 90, size = 10, hjust = 0.5)) +  # Customize nested axis text appearance
...
pp/p +plot_layout(heights = c(0.2,0.8)) & 
  theme(legend.position = c(-0.12,1.11),
        plot.margin = unit(c(0, 0.1, 0.1, 0), "cm")) #unit(c(top, right, bottom, left), "cm")

# Save the final combined plot as a high-resolution image
ggsave(here("data","Figure2.png"), dpi = 600, units = "cm", width = 20, height = 18)  # Save figure at 600 DPI
